...
export default function Home() {
  return <><Header /><main>
    ...
    <section className="home-section news-section">
      ...
      <div className="home-news">{recentNews.slice(0,4).map((item)=><article className={item.image ? "" : "news-without-image"} key={item.id}><div className="news-copy"><div><span className="news-type">{item.category}</span><span className="news-date">{item.year}</span></div><p>{item.href ? <a href={sitePath(item.href)}>{item.title}</a> : item.title}</p></div>{item.image && <img src={sitePath(item.image)} alt="" />}</article>)}</div></div>
    </section>
