...
assert.match(peopleHtml, /Ph\.D\. @ UIUC/);
  ...
  assert.match(piHtml, /AI Star Fellowship \(AI최고급신진연구자지원\)/);
  assert.doesNotMatch(piHtml, /Scientific AI를 위한 과학 도메인 특화 계층적 Knowledge Graph 기반 RAG 기술/);
  ...
  assert.equal((publicationsHtml.match(/class="paper-row"/g) ?? []).length, 62);
  ...
  assert.ok(publicationsHtml.indexOf("2026-cikm-tracer") < publicationsHtml.indexOf("2026-cikm-keir-compact-sparse-spaces"));
  assert.ok(publicationsHtml.indexOf("2026-cikm-keir-compact-sparse-spaces") < publicationsHtml.indexOf("2026-colm-reward-factorization"));
  ...
  assert.match(publicationsHtml, /https:\/\/idealab-ku\.github\.io\/idealab-website\/media\/publications\/2026\/papers\/2026-cikm-spectral-decoupling\.pdf/);
  assert.match(publicationsHtml, /https:\/\/arxiv\.org\/pdf\/2608\.26604/);
...
test("restores the complete news archive and related links", async () => {
  ...
  assert.ok(html.indexOf("2026-cikm-tracer") < html.indexOf("2026-cikm-keir-compact-sparse-spaces"));
  assert.ok(html.indexOf("2026-cikm-keir-compact-sparse-spaces") < html.indexOf("2026-colm-reward-factorization"));
...
test("presents research areas, major projects, and verified collaborators", async () => {
  ...
  assert.equal((html.match(/class="research-project-item"/g) ?? []).length, 5);
  assert.equal((html.match(/class="research-project-role">책임</g) ?? []).length, 3);
  assert.equal((html.match(/class="research-project-role">참여</g) ?? []).length, 2);
  ...
  assert.match(html, /AI Star Fellowship \(AI최고급신진연구자지원\)/);
  assert.match(html, /IITP/);
  assert.match(html, /2026–2031/);
...
test("keeps event captions matched to the verified local photographs", async () => {
  ...
  assert.match(css, /\.detail-page \.workshop-sublist li\{[^}]*color:inherit/);
  assert.match(css, /\.detail-page \.workshop-sublist strong\{[^}]*color:inherit/);
});
